#some codes are from TrafficStream, Thank you.
...
result_1 = {3:{"new_mae":{}, "new_mape":{}, "new_rmse":{}}, 6:{"new_mae":{}, "new_mape":{}, "new_rmse":{}}, 12:{"new_mae":{}, "new_mape":{}, "new_rmse":{}}}
result_2 = {3:{"old_mae":{}, "old_mape":{}, "old_rmse":{}}, 6:{"old_mae":{}, "old_mape":{}, "old_rmse":{}}, 12:{"old_mae":{}, "old_mape":{}, "old_rmse":{}}}
pin_memory = False
n_work = 16
...
def train(inputs, args):
    ...
    if args.strategy == 'incremental' and args.year > args.begin_year :
        path6=osp.join('./data/cluster',str(args.year)+"_cluster.npz")
        scor=np.load(path6)
        ...
        lenght_data=len(train_loader)
        mask1 = random.sample(range(lenght_data),int(lenght_data*0.8))     
        train_loader=train_loader[mask1]
        graph = nx.Graph()
        ...
        path6=osp.join('./data/cluster',str(args.year)+"_cluster.npz")
        scor=np.load(path6)
        lenght_data=len(inputs["train_x"])

        mask1 = random.sample(range(lenght_data),int(lenght_data*0.5))
    ...
    if args.init == True and args.year > args.begin_year:
        ...
        if args.ewc:
            ...
            model = EWC(gnn_model, args.adj, scor,args.ewc_lambda, args.ewc_strategy)
            ewc_loader = DataLoader(TrafficDataset(inputs, "train",scor['train']), batch_size=args.batch_size, shuffle=False, pin_memory=pin_memory, num_workers=n_work)
    ...
    for year in range(args.begin_year, args.end_year+1):
        ...
        if year > args.begin_year and args.strategy == "incremental":
            ...
            if 1:
                ...
                vars(args)['new_node']=node_list
                vars(args)['old_node']=list(range(0,pre_node_size))
            if args.detect:
                ...
                # 10% of current graph size will be sampled
                vars(args)["topk"] = int(0.1*args.graph_size) 
                influence_node_list,replay1= detect.get_differeNl(args)
                node_list.extend(list(influence_node_list))
                node_list.extend(list(replay1))
                ...
                if i in result:
                    if j in result[i]:
                        if year in result[i][j]:
                            cnas=cnas+result[i][j][year]
                            info+="{:.2f}\t".format(result[i][j][year])
            info+="{:.2f}\t".format(cnas/(args.end_year-args.begin_year+1))
            logger.info("{}\t{}\t".format(i,j) + info)
    for i in [3, 6, 12]:
        for j in ['old_mae', 'old_rmse', 'old_mape']:
            info = ""
            cnas=0
            for year in range(args.begin_year, args.end_year+1):
                if i in result_2:
                    if j in result_2[i]:
                        if year in result_2[i][j]:
                            if year!=2011:
                                cnas=cnas+result_2[i][j][year]
                                info+="{:.2f}\t".format(result_2[i][j][year])
                            else:
                                info+="{:.2f}\t".format(0)
            info+="{:.2f}\t".format(cnas/(args.end_year-args.begin_year))
            logger.info("{}\t{}\t".format(i,j) + info)
    for i in [3, 6, 12]:
        for j in ['new_mae', 'new_rmse', 'new_mape']:
            info = ""
            cnas=0
            for year in range(args.begin_year, args.end_year+1):
                if i in result_1:
                    if j in result_1[i]:
                        if year in result_1[i][j]:
                            if year!=2011:
                                cnas=cnas+result_1[i][j][year]
                                info+="{:.2f}\t".format(result_1[i][j][year])
                            else:
                                info+="{:.2f}\t".format(0)
            info+="{:.2f}\t".format(cnas/(args.end_year-args.begin_year))
            logger.info("{}\t{}\t".format(i,j) + info)
